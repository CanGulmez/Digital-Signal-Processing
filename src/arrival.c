...
int dsp_arrival_music(const DspArrival *arrival)
{
	...
	mic_t mics = arrival->mics;
	src_t sources = arrival->sources;
	double radius = arrival->radius;
	double freq = arrival->freq;
	gsl_matrix *X, *R, *evec;
	gsl_vector *eval;
	...
	if ((mics < 2) || (sources < 1) || (radius <= 0.0) || 
		(freq <= 1.0) || (mics > MAX_MICS) || (sources > mics))
		return -1;

	/* Validate the microfon samples. */
	for (i = 0; i < mics; i++)
	{
		if (!IS_SAMPLE(arrival->samples[i]))
			return -1;
	}

	/* Allocate the required blocks. */
	X = gsl_matrix_alloc(mics, arrival->samples[0]->length);
	R = gsl_matrix_alloc(mics, mics);
	eval = gsl_vector_alloc(mics);
	evec = gsl_matrix_alloc(mics, mics);
	w = gsl_eigen_symmv_alloc(mics);
	En = gsl_matrix_complex_alloc(mics, mics - sources);
	a = gsl_vector_complex_alloc(mics);
	EnH_a = gsl_vector_complex_alloc(mics - sources);
	EnEnHa = gsl_vector_complex_alloc(mics);
	...
	gsl_blas_dgemm(CblasNoTrans, CblasTrans, 1.0 / arrival->samples[0]->length, X, X, 0.0, R);
	...
	for (theta = 0; theta < 360; theta++) 
	{
		...
		for (i = 0; i < mics; i++) 
		{
			phi_i = 2.0 * M_PI * i / mics;
			k = 2.0 * M_PI * freq / SOUND_SPEED;
		...
		gsl_blas_zgemv(CblasConjTrans, GSL_COMPLEX_ONE, En, a, GSL_COMPLEX_ZERO, EnH_a);
		gsl_blas_zgemv(CblasNoTrans, GSL_COMPLEX_ONE, En, EnH_a, GSL_COMPLEX_ZERO, EnEnHa);
